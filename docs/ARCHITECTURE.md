...
- `storage` performs conservative, read-only Ryujinx container discovery from a selected root and reports diagnostics.
...
The implemented discovery flow is UI-selected root → discovery service → storage provider → normalized saves and diagnostics → UI. A future editing flow is UI intent → application service → verified backup and transactional writer → plugin transform/validation. Plugins may parse a private game model, but the core must not import game format modules. The transaction owner must validate plugin-provided output before committing. Provider paths and plugin file lists are untrusted inputs requiring containment checks before copying or writing.
